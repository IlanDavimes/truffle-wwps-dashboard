...
function buildSections(c: Consultant, revealed: boolean): Paragraph[] {
  const out: Paragraph[] = []
  const fullName = revealed ? `${c.firstName} ${c.surname}` : `${c.firstName} [surname concealed]`

  out.push(
    new Paragraph({
      children: [new TextRun({ text: 'Truffle Consulting · Consultant Profile', size: 18, color: MUTED })],
      spacing: { after: 120 },
    }),
  )
  out.push(
...
function makeFilename(c: Consultant, revealed: boolean): string {
  const surname = revealed ? c.surname : 'redacted'
...
export async function exportConsultantDocx({ consultant, revealed }: ExportOptions): Promise<void> {
  const doc = new Document({
    creator: 'Truffle Consulting',
    title: `${consultant.firstName} ${revealed ? consultant.surname : ''} · Truffle CV`.trim(),
    ...
    sections: [
      {
        properties: {},
        children: buildSections(consultant, revealed),
      },
    ],
  })
  const blob = await Packer.toBlob(doc)
  triggerDownload(blob, makeFilename(consultant, revealed))
}
